...
const gui = new GUI()
const debugObject = {DColor: '#0000ff', SColor: '#8888ff'}
...
const material = new THREE.ShaderMaterial({
    ...
    uniforms:
    {
        ...
        uTime: {value: 0},
        uDColor: {value: new THREE.Color(debugObject.DColor)},
        uSColor: {value: new THREE.Color(debugObject.SColor)},
    }
})
gui.add(material.uniforms.uElevation, 'value').min(0).max(2).step(0.01).name('Elevation')
gui.add(material, 'wireframe')
gui.add(material.uniforms.uFrequency.value, 'x').min(0).max(10).step(0.001)
gui.add(material.uniforms.uFrequency.value, 'y').min(0).max(10).step(0.001)

gui.addColor(debugObject, 'DColor').onChange(() => {
    material.uniforms.uDColor.value.set(debugObject.DColor)
})

gui.addColor(debugObject, 'SColor').onChange(() => {
    material.uniforms.uSColor.value.set(debugObject.SColor)
})
